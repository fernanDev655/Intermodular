...
import com.example.concesionario.repository.BaseRepository;
import com.example.concesionario.repository.VehiculoImagenRepository;
import com.example.concesionario.mapper.RowMapper;

public class VehiculoController extends BaseRepository<Vehiculo> {

    public VehiculoController(Connection con) {
        super(con, new VehiculoMapper());
    }

    public VehiculoController(Connection con, RowMapper<Vehiculo> mapper) {
        super(con, mapper);
    }
    ...
    public VehiculoDetalleResponse findDetalle(int vehiculoId, Integer userId) {
        // FIX: sin coma sobrante, sin LEFT JOIN a tabla 'votos' inexistente
        String sql = """
            SELECT
                v.id,
                v.marca,
